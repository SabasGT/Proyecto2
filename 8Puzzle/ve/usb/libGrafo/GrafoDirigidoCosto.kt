...
public class GrafoDirigidoCosto : Grafo {
    private var adjLista: MutableList<MutableList<ArcoCosto>>  // Contenedor de los elementos del grafo
    private var listaTableros: MutableList<MutableList<Int>> = MutableList(0){mutableListOf()}   // Lista de los tableros creados durante la corrida

    // Se construye un grafo a partir del número de vértices
    constructor(/*numDeVertices: Int*/) {
        this.adjLista = MutableList(1){mutableListOf()}
    }
    ...
    */
    ...
    fun agregarArcoCosto(a: ArcoCosto): Boolean {
        ...
        if (!this.adjLista[inicio].map{a -> a.y()}.contains(fin)){
    ...
    // Retorna el estado objetivo en forma de lista.
    fun meta(): List<List<Int>> {
        val meta: List<List<Int>> = listOf(listOf(1,2,3), listOf(4,5,6), listOf(7,8,0))
        return meta
    }
